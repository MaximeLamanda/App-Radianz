...
- API Next (auth identique aux autres routes matching) : **`GET /api/matching-v5/features`** — paramètres **`code_insee`** (obligatoire), **`limit`** (plafonné à 5000), bbox optionnelle **`minLat`**, **`maxLat`**, **`minLng`**, **`maxLng`** — réponse **`FeatureCollection`** consommable par `parseMatchingV5GeoJsonFeatureCollection`.
