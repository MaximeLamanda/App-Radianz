#!/usr/bin/env python3
"""
Import polygones OSM landuse=* (.osm.pbf) vers Postgres (public.osm_landuse_areas).

  pip install -r data-pipeline/python/requirements.txt
  python3 data-pipeline/matching_v5/import_osm_landuse.py --input chemin.osm.pbf --truncate
"""
...
from osm_landuse_v5 import qualified_osm_landuse_table  # noqa: E402
from osm_poi_v5 import tags_dict  # noqa: E402

DEFAULT_ALLOWED_LANDUSE = (
    "industrial",
    "commercial",
    "retail",
    "residential",
...
def _tags_stored(tags: dict[str, str], landuse_raw: str) -> dict[str, str]:
    out: dict[str, str] = {"landuse": landuse_raw}
    for k, v in tags.items():
        lk = str(k).strip()
        if not lk or lk == "landuse":
            continue
        if lk in {"name", "operator"} or lk.startswith("source"):
            sv = str(v).strip()
            if sv:
                out[lk] = sv
    return out
...
def build_landuse_matcher(allowed_csv: str) -> Callable[[str], str | None]:
    if not (allowed_csv or "").strip():
        allowed = list(DEFAULT_ALLOWED_LANDUSE)
    else:
...
class PbfLanduseCollector(osmium.SimpleHandler):
    ...
    def _emit(self, osm_type: str, osm_id: int, geom: Any, tags: dict[str, str], landuse_raw: str) -> None:
    ...
    def way(self, w: osmium.osm.Way) -> None:
        ...
        try:
            ...
            self.skipped_geom += 1
            return
        self._emit("w", int(w.id), geom, tags, lu)
    ...
    def area(self, a: osmium.osm.Area) -> None:
        ...
        try:
            ...
            self.skipped_geom += 1
            return
        self._emit("r", int(a.id), geom, tags, lu)


def apply_osm_landuse_schema_sql(cur: Any) -> None:
    sql_path = REPO_ROOT / "data-pipeline" / "sql" / "006_osm_landuse_areas.sql"
...
def insert_batch(
    cur: Any,
    qualified: str,
    batch: list[tuple[str, int, str, str, dict[str, str]]],
) -> None:
    import psycopg2.extras

    rows = [(ot, oid, wkt, lu, psycopg2.extras.Json(tags)) for ot, oid, wkt, lu, tags in batch]
    psycopg2.extras.execute_batch(
        cur,
        f"""
        INSERT INTO {qualified} (osm_type, osm_id, geom, landuse, tags)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Import polygones landuse OSM (PBF)")
    ...
    try:
        ...
        with conn_s.cursor() as cur:
            apply_osm_landuse_schema_sql(cur)
    finally:
        conn_s.close()
    print(f"[osm_landuse] Schéma vérifié ({qualified_osm_landuse_table()})", flush=True)
    ...
    qualified = qualified_osm_landuse_table()
    ...
    try:
        ...
        conn.rollback()
        print(f"[osm_landuse] Erreur: {e}", file=sys.stderr)
        return 1
    finally:
        conn.close()
    print(
        f"[osm_landuse] Terminé — upsert {inserted} | ways {collector.ways_kept} | "
