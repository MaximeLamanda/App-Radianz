...
/**
 * Bâtiments groupe BDNB pour Pessac uniquement : table `public.bdnb_pessac_geom_raw`
 * (remplie par l’import sans jointure, surface groupe &gt; 1000 m² — INSEE **33318**, voir scripts/import-bdnb-postgres.mjs).
 */
export async function GET(request: NextRequest) {
  ...
  try {
    const { rows } = await client.query<{
      ...
      FROM public.bdnb_pessac_geom_raw
    ...
    console.error("[bdnb-pessac-raw/bbox]", err);
    const msg = err instanceof Error ? err.message : String(err);
    if (msg.includes("does not exist") || msg.includes("n'existe pas")) {
      return NextResponse.json(
        {
          error: "Table bdnb_pessac_geom_raw absente. Lance npm run import:bdnb-dep33 (ou l’import BDNB).",
          detail: msg,
        },
        { status: 503 }
      );
